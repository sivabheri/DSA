...
class Solution:
    def decrypt(self, code, k):
        ...
        if k==0:
            return res
        if k>0:
            wsum = sum(code[(i + 1) % n] for i in range(k))
            res[0] = wsum
            left,right = 1,k+1
            while(left<n):
                wsum += code[right%n] - code[left]
                res[left] = wsum
                left+=1
                right+=1
            return res
...
if __name__ == '__main__':
	...
	k = int(input())
	decrypt_arr = obj.decrypt(arr,k)

	print(f'The decrypted array is : {decrypt_arr}')
